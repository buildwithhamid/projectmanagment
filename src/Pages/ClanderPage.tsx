...
export default function CalendarPage() {
  ...
  const allTasks = useMemo(() => {
    return Object.entries(taskCache).flatMap(([projectId, data]) => {
      const project = projects.find((p) => p.id === projectId);
      return (data?.tasks || []).map((task) => ({
        ...task,
        projectId,
        projectName: data?.title || project?.title || "Untitled Project",
              ...
              {tasksForDay.map((task) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    <div className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-background text-lg">
